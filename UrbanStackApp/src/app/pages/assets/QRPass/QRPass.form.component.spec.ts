...
describe('QRPassForm', () => {
	...
	let ViewControllerMock = mockView();
  	...
  	beforeEach(async(() => {
    	...
    	TestBed.configureTestingModule({
			...
			imports: [
				IonicModule.forRoot(QRPassForm)
			],
			providers: [
				...
				{ provide: QRPassService, useClass:serviceQRPass },
  	...
  	beforeEach(() => {
		assetType = 'QRPass';
		spyOn(serviceQRPass.prototype,'get').and.callThrough();
    	fixture = TestBed.createComponent(QRPassForm);
    	component = fixture.componentInstance;
  	});

  	it('should create QRPassForm component', () => {
		expect(component instanceof QRPassForm).toBe(true);
	});

	it('should have correct asset identifier property', () => {
		expect(component.assetIdentifierProperty).toEqual('QRPassId');
	})

	it('should have correct title',() => {
		component.formType = 'Add';
		let expectedTitle = 'Add QRPass';
	...
	it('should detect if update form is opened', fakeAsync(() => {
		...
		expect(LoadingControllerMock.prototype.create).toHaveBeenCalledWith({
			content: 'Fetching QRPass001'
		});

		expect(LoadingMock.prototype.present).toHaveBeenCalled();

		expect(FormMock.prototype.get).toHaveBeenCalledWith('QRPassId');
	...
	it('should add an asset', fakeAsync(() => {
		...
		spyOn(LoadingMock.prototype,'dismiss');
		spyOn(LoadingMock.prototype,'present');
		spyOn(serviceQRPass.prototype,'add').and.returnValue(new Promise((resolve,reject)=>{resolve()}));
		...
		expect(LoadingMock.prototype.present).toHaveBeenCalled();
		expect(serviceQRPass.prototype.add).toHaveBeenCalled();
	...
	it('should throw an error for invalid add asset', fakeAsync(() => {
		...
		spyOn(serviceQRPass.prototype,'add').and.returnValue(new Promise((resolve,reject) => {
			reject({_body:json})
		}));
		spyOn(ViewControllerMock,'dismiss');
		component.addAsset();
	...
	it('should update an asset', fakeAsync(() => {
		...
		spyOn(LoadingMock.prototype,'dismiss');
		spyOn(LoadingMock.prototype,'present');
		spyOn(serviceQRPass.prototype,'update').and.returnValue(new Promise((resolve,reject)=>{resolve()}));
		spyOn(ViewControllerMock,'dismiss');
		spyOn(FormMock.prototype,'get').and.returnValue(formObjectMock);
		spyOn(FormObjectMock.prototype,'enable');
		...
		expect(LoadingMock.prototype.present).toHaveBeenCalled();
		expect(serviceQRPass.prototype.update).toHaveBeenCalled();
		expect(ViewControllerMock.dismiss).toHaveBeenCalled();
	}));
	...
	it('should throw an error for invalid update asset', fakeAsync(() => {
		...
		spyOn(LoadingMock.prototype,'present');
		let json = JSON.stringify({error:{message:'Some error'}});
		spyOn(serviceQRPass.prototype,'update').and.returnValue(new Promise((resolve,reject) => {
			reject({_body:json})
		}));
		spyOn(ViewControllerMock,'dismiss');
		component.updateAsset();
