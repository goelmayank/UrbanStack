...
class serviceMobilityAsset{
	get(assetId){
		return new Promise((resolve,reject)=>{
			resolve(MobilityAssetOne);
		});
	}
	add(form){
		return new Promise((resolve,reject) => {
...
describe('MobilityAssetForm', () => {
	...
	let ViewControllerMock = mockView();
  	...
  	beforeEach(async(() => {
    	...
    	TestBed.configureTestingModule({
			...
			imports: [
				IonicModule.forRoot(MobilityAssetForm)
			],
			providers: [
				...
				{ provide: MobilityAssetService, useClass:serviceMobilityAsset },
  	...
  	beforeEach(() => {
		...
		spyOn(serviceMobilityAsset.prototype,'get').and.callThrough();
    	fixture = TestBed.createComponent(MobilityAssetForm);
    	component = fixture.componentInstance;
  	});

  	it('should create MobilityAssetForm component', () => {
		expect(component instanceof MobilityAssetForm).toBe(true);
	});

	it('should have correct asset identifier property', () => {
		expect(component.assetIdentifierProperty).toEqual('MiD');
	})

	it('should have correct title',() => {
		component.formType = 'Add';
		let expectedTitle = 'Add MobilityAsset';
	...
	it('should detect if update form is opened', fakeAsync(() => {
		...
		component.assetId = 'MobilityAsset001'
		fixture.detectChanges();
		...
		expect(LoadingControllerMock.prototype.create).toHaveBeenCalledWith({
			content: 'Fetching MobilityAsset001'
		});

		expect(LoadingMock.prototype.present).toHaveBeenCalled();

		expect(FormMock.prototype.get).toHaveBeenCalledWith('MiD');
	...
	it('should add an asset', fakeAsync(() => {
		...
		spyOn(LoadingMock.prototype,'dismiss');
		spyOn(LoadingMock.prototype,'present');
		spyOn(serviceMobilityAsset.prototype,'add').and.returnValue(new Promise((resolve,reject)=>{resolve()}));
		...
		expect(LoadingMock.prototype.present).toHaveBeenCalled();
		expect(serviceMobilityAsset.prototype.add).toHaveBeenCalled();
	...
	it('should throw an error for invalid add asset', fakeAsync(() => {
		...
		spyOn(serviceMobilityAsset.prototype,'add').and.returnValue(new Promise((resolve,reject) => {
			reject({_body:json})
		}));
		spyOn(ViewControllerMock,'dismiss');
		component.addAsset();
	...
	it('should update an asset', fakeAsync(() => {
		...
		spyOn(LoadingMock.prototype,'dismiss');
		spyOn(LoadingMock.prototype,'present');
		spyOn(serviceMobilityAsset.prototype,'update').and.returnValue(new Promise((resolve,reject)=>{resolve()}));
		spyOn(ViewControllerMock,'dismiss');
		spyOn(FormMock.prototype,'get').and.returnValue(formObjectMock);
		spyOn(FormObjectMock.prototype,'enable');
		...
		expect(LoadingMock.prototype.present).toHaveBeenCalled();
		expect(serviceMobilityAsset.prototype.update).toHaveBeenCalled();
		expect(ViewControllerMock.dismiss).toHaveBeenCalled();
	}));
	...
	it('should throw an error for invalid update asset', fakeAsync(() => {
		...
		spyOn(LoadingMock.prototype,'present');
		let json = JSON.stringify({error:{message:'Some error'}});
		spyOn(serviceMobilityAsset.prototype,'update').and.returnValue(new Promise((resolve,reject) => {
			reject({_body:json})
		}));
		spyOn(ViewControllerMock,'dismiss');
		component.updateAsset();
