...
export class PassengerPage {
	...
	constructor(
		...
		this.servicePassenger.getAll().then((participants) => {
	...
	ionViewWillEnter(): any {
		...
		let loading = this.loadingCtrl.create({
			content: 'Fetching all Passenger participants'
		});
		loading.present();
		return this.servicePassenger.getAll().then((participants) => {
			this.currentItems = participants;
			this.items = participants;
			this.itemsLoaded = true;
			loading.dismiss();
	...
	viewParticipant(participantId){
		let modal = this.modalCtrl.create(ParticipantViewPage, {
			participantId: participantId, participantType: 'Passenger'
		});
		modal.present();
	}

	addParticipant(){
		let modal = this.modalCtrl.create(PassengerForm, {
			participantId: 'email', participantType: 'Passenger', properties: this.properties, formType: 'Add'
		});
		modal.present();
	...
	updateParticipant(participantId){
		let modal = this.modalCtrl.create(PassengerForm, {
			participantId: participantId, participantType: 'Passenger',properties: this.properties, formType: 'Update'
		});
		modal.present();
		...
		let confirm = this.alertCtrl.create({
			...
			buttons: [{
				...
				handler: () => {
					...
					let loading = this.loadingCtrl.create({
						content: 'Deleting Passenger...'
					});

					loading.present(); // Show loading animation

					this.servicePassenger.delete(participantId).then(() => {
